(() => {
    ...
    const setTotals = (count, subtotal) => {
        ...
        subtotalEls.forEach((el) => {
            el.textContent = `${formatPrice(subtotal)} تومان`;
        });
    };
    ...
    const updateItem = (item, data) => {
        ...
        if (quantity) quantity.textContent = formatPrice(data.quantity);
    ...
    root.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            if (form.dataset.cartAction === "clear") {
                root.querySelectorAll("[data-cart-item]").forEach((el) => el.remove());
            }
    ...
    root.addEventListener("click", async (event) => {
        ...
        const current = Number(item.querySelector("[data-quantity-value]")?.dataset.value || item.querySelector("[data-quantity-value]")?.textContent.replace(/\D/g, "") || 1);
        const delta = Number(button.dataset.quantityButton);
    ...
    root.querySelectorAll("[data-quantity-value]").forEach((el) => {
