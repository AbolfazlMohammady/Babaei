...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        primary_images = ProductImage.objects.filter(product_id=OuterRef("pk"), image_type=ProductImage.ImageType.PRIMARY).order_by("sort_order", "id")
        ...
        primary_image_alt = Subquery(primary_images.values("alt_text")[:1], output_field=CharField(max_length=180))
        variant_price = Subquery(
            ProductVariant.objects.filter(product_id=OuterRef("pk"), is_active=True).order_by("price", "id").values("price")[:1],
            output_field=IntegerField(),
        )
        context["home_products"] = (
            Product.objects.filter(is_active=True, category__is_active=True)
            .select_related("category")
            .annotate(listed_price=variant_price, primary_image_url=primary_image_url, primary_image_alt=primary_image_alt)
