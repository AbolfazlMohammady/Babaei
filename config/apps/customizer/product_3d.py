...
def product_images(product):
    return list(
        product.images.all()
        .only("id", "image", "image_type", "sort_order")
        .order_by("sort_order", "id")[:MAX_SOURCE_IMAGES]
    )


def source_signature(product_images):
    value = "|".join(f"{item.id}:{item.image.name}" for item in product_images)
...
def _prepare_source(product_image):
    ...
    if has_alpha:
        ...
        processed = _remove_bg(raw, product_image.image.name)
        background_removed = True
...
def _create_meshy_task(image_data_uris):
    ...
    endpoint = f"{MESHY_URL}/multi-image-to-3d" if len(image_data_uris) > 1 else f"{MESHY_URL}/image-to-3d"
    payload = {
...
def get_meshy_task(task_id):
    ...
    _, response = _json_request(
        f"{MESHY_URL}/image-to-3d/{task_id}",
...
def prepare_product_asset(asset: Product3DAsset):
    ...
    for index, product_image in enumerate(images):
        ...
        source = Product3DSource.objects.create(
            asset=asset,
            product_image=product_image,
            sort_order=index,
            background_removed=True,
        )
        source.processed_image.save(
            f"source-{product_image.id}.png",
...
def refresh_product_asset(asset: Product3DAsset):
    response = get_meshy_task(asset.task_id)
    ...
    model_bytes = download_binary(model_url)
    asset.model_3d.save("product.glb", ContentFile(model_bytes), save=False)
