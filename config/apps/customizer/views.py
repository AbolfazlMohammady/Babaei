...
def _model_url(request, view):
    """Prefer an uploaded product-specific GLB/GLTF, then an explicitly configured CDN URL."""
...
class DesignerPageView(View):
    ...
    def get(self, request, slug):
        ...
        primary_image = (
            ProductImage.objects.filter(product=product, image_type=ProductImage.ImageType.PRIMARY)
            .only("image", "alt_text")
            .first()
        )
        ...
        model_count = sum(1 for item in view_data if item["model"])
        context = {
            "product": product,
            ...
            "designer_model_count": model_count,
